...
class HggBaseProcessor(processor.ProcessorABC):  # type: ignore
    ...
    def process(self, events: awkward.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            # FatJet = Fatjet_preselection(self, FatJet, diphotons)
            # FatJet = awkward.pad_none(FatJet, 1)
            # Hgg_score = FatJet.globalParT3_Xaa
            # Hgg_score = awkward.fill_none(Hgg_score, -999)
            # print("score:", len(Hgg_score), Hgg_score)
            # print("pt:", len(diphotons["pt"]), diphotons["pt"])
            # diphotons["Hgg_score"] = Hgg_score
            ...
            if self.data_kind == "mc":
                ...
                diphotons['Gensublead_eta'] = Gensublead_eta
                ...
                diphotons['Gensublead_phi'] = Gensublead_phi
            ...
            FatJet = events.FatJet
            FatJet = FatJet[Fatjet_preselection(self, FatJet, diphotons)]
            FatJet = FatJet[awkward.argsort(FatJet.globalParT3_Xaa, ascending=False)]

            # Score
            Hgg_score = choose_jet(FatJet.globalParT3_Xaa, 0, -999)
            QCDb_score = choose_jet(FatJet.globalParT3_QCDb, 0, -999)
            QCDbb_score = choose_jet(FatJet.globalParT3_QCDbb, 0, -999)
            QCDc_score = choose_jet(FatJet.globalParT3_QCDc, 0, -999)
            QCDcc_score = choose_jet(FatJet.globalParT3_QCDcc, 0, -999)
            QCDothers_score = choose_jet(FatJet.globalParT3_QCDothers, 0, -999)


            FatJet_pt = choose_jet(FatJet.pt, 0, -999)
            FatJet_eta = choose_jet(FatJet.eta, 0, -999)
            FatJet_mass = choose_jet(FatJet.mass, 0, -999)
            FatJet_phi = choose_jet(FatJet.phi, 0, -999)
            FatJet_softdropmass = choose_jet(FatJet.msoftdrop, 0, -999)
            FatJet_massCorrGeneric = choose_jet(FatJet.globalParT3_massCorrGeneric, 0, -999)
            FatJet_massCorrQCDb = choose_jet(FatJet.globalParT3_massCorrQCDb, 0, -999)
            FatJet_massCorrQCDbb = choose_jet(FatJet.globalParT3_massCorrQCDbb, 0, -999)
            FatJet_massCorrQCDc = choose_jet(FatJet.globalParT3_massCorrQCDc, 0, -999)
            FatJet_massCorrQCDcc = choose_jet(FatJet.globalParT3_massCorrQCDcc, 0, -999)
            FatJet_massCorrQCDothers = choose_jet(FatJet.globalParT3_massCorrQCDothers, 0, -999)
            FatJet_massCorrXaa = choose_jet(FatJet.globalParT3_massCorrXaa, 0, -999)
            FatJet_rawFactor = choose_jet(FatJet.rawFactor, 0, -999)
            diphotons["Hgg_score"] = Hgg_score
            diphotons["FatJet_pt"] = FatJet_pt
            diphotons["FatJet_eta"] = FatJet_eta
            diphotons["FatJet_mass"] = FatJet_mass
            diphotons["FatJet_phi"] = FatJet_phi
            diphotons["FatJet_softdropmass"] = FatJet_softdropmass
            diphotons["n_FatJets"] = awkward.num(FatJet)

            # mass variables
            diphotons["FatJet_massCorrGeneric"] = FatJet_massCorrGeneric
            diphotons["FatJet_massCorrQCDb"] = FatJet_massCorrQCDb
            diphotons["FatJet_massCorrQCDbb"] = FatJet_massCorrQCDbb
            diphotons["FatJet_massCorrQCDc"] = FatJet_massCorrQCDc
            diphotons["FatJet_massCorrQCDcc"] = FatJet_massCorrQCDcc
            diphotons["FatJet_massCorrQCDothers"] = FatJet_massCorrQCDothers
            diphotons["FatJet_massCorrXaa"] = FatJet_massCorrXaa
            diphotons["FatJet_rawFactor"] = FatJet_rawFactor

            # Score
            diphotons["QCDb_score"] = QCDb_score
            diphotons["QCDbb_score"] = QCDbb_score
            diphotons["QCDc_score"] = QCDc_score
            diphotons["QCDcc_score"] = QCDcc_score
            diphotons["QCDothers_score"] = QCDothers_score
